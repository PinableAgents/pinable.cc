...
- **The hero product shot has no second surface behind it — don't add one back.** `.he-shot-band` renders the app screenshot inside fake macOS window chrome (`.he-shot-card`: 880px, `border-bottom: none`, cropped by the band's bottom edge), and the card's own 1px border plus `--pa-shadow-lg` carry all of the separation it needs. Two backdrops were built and removed: first two centred panels (`--vp-c-bg-soft` left, `--vp-c-bg-alt` right, split at the centre with a `border-right`) — the card covers that seam at every width, so the split was never visible and all it produced was one side reading a shade darker than the other; then a single mat spanning the band — still concentric with the card but starting 55% of the way down, so its top edge crossed the middle of the shot and emerged either side as two stubs. The root cause is the palette, not the geometry: with colour reserved for status, a second surface sits only ~10/255 off `--vp-c-bg`, so it reads as a smudge rather than as a designed layer — and at 90% width (1062px) it aligns with neither the card's 880px nor the page's 1180px. If the shot ever needs more presence, change the card (border weight, shadow, a real tinted surface behind *it*), not the empty space around it.
